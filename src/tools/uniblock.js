...
async function rpc(chainId, method, params) {
  const res = await fetch(`${BASE}/json-rpc?chainId=${chainId}`, {
    method: 'POST',
...
async function getABI(address, chainId) {
  const res = await fetch(`${BASE}/scan/contract-abi?chainId=${chainId}&address=${address}`, { headers: headers() });
...
export async function fetchSourceCode(contractAddress, chainId) {
  const res = await fetch(`${BASE}/scan/contract-source-code?chainId=${chainId}&contractAddress=${contractAddress}`, { headers: headers() });
